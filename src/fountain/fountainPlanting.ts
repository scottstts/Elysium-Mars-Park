...
import type { PlantingPalette } from '../vegetation/planting'
 ...
 * ferns and broadleaf at the back, flower at the shoulders, trailing cover
...
export function plantFountainBays(palette: PlantingPalette, rng: Rng): number {
  ...
  for (const bay of planterBays()) {
    ...
    // Sedge fills between, and cover closes the soil.
    const sedge = rng.int(10, 14)
    for (let i = 0; i < sedge; i++) {
      palette.sedge.add(
        point(rng.range(0.15, 0.9), rng.range(0.04, 0.96)).add(new Vector3(0, -0.03, 0)),
        rng.range(0, Math.PI * 2),
        rng.range(0.7, 1.2),
        rng.range(-0.12, 0.12),
        rng.range(-0.12, 0.12),
    ...
    const cover = rng.int(12, 18)
    for (let i = 0; i < cover; i++) {
      palette.cover.add(
        point(rng.range(0.08, 0.95), rng.range(0.02, 0.98)).add(new Vector3(0, -0.02, 0)),
        rng.range(0, Math.PI * 2),
        rng.range(0.7, 1.1),
      )
      planted++
